    ...
    public function getForDataTable($status = 1, $trashed = false)
    {
        ...
        if ($trashed == "true") {
            return User::onlyTrashed()
                ->select(['id', 'name', 'account_id', 'email', 'status', 'confirmed', 'mobile', 'deviceId', 'deleted_at'])
                ->get();
        }

        return User::select(['id', 'name','account_id', 'email', 'status', 'confirmed', 'mobile', 'deviceId', 'deleted_at'])
            ->where('status', $status)
            ->get();
    }
